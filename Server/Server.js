...
app.post('/api/login', async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(
            'SELECT id, username FROM users WHERE username = ? AND master_password = ?',
        ...
        if (rows.length > 0) {
            res.json({ success: true, userId: rows[0].id, user: rows[0].username });
        } else {
...
app.post('/api/user/passwords', async (req, res) => {
    try {
        ...
        const [result] = await pool.execute(
            'INSERT INTO entries (user_id, site, username, password) VALUES (?, ?, ?, ?)',
